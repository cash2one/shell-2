...
if [[ $app == 'hzapi' ]];then
    ...
    file_name='RELEASE_VERSION_HaozuAPI_BETA'
    dir_ext='haozu-mobile-api/'
elif [[ $app == 'ajkapi' ]];then
    release_version_file_dir='/home/www/conf/'
    file_name='RELEASE_VERSION_AnjukeAPI_BETA'
    dir_ext='anjuke-mobile-api/app-api/'
elif [[ $app == 'hzold' ]];then
    release_version_file_dir='/home/www/conf/'
    file_name='RELEASE_VERSION_HaozuAPI_V3.0_BETA'
    dir_ext='haozu-mobile-api-v3.0/'
elif [[ $app == 'jpapi' ]];then
    release_version_file_dir='/home/www/conf/'
    file_name='RELEASE_VERSION_JinpuAPI_BETA'
    ...
    release_version_file_dir='/home/www/config/machine/'
    file_name='API_RELEASE_VERSION_BETA'
    dir_ext='wechat/'
    ...
    release_version_file_dir='/home/www/conf/'
    file_name='CMS_USER_RELEASE_VERSION_BETA'
    dir_ext='cms-user/'
else
    release_version_file_dir='/home/www/conf/'
    file_name=$app'_RELEASE_VERSION_BETA'
    dir_ext=$app/
fi
...
if [[ $app == "anjuke" ]];then
    if [ -d "/home/www/release/v2/${dir_ext}${version}" ]; then
        ...
        if [[ $if_ok == "4.0K" ]]; then
            echo "${machine_tag} ${app} ${version} not exists; switch beta version fail !"
            exit;
        fi
    else
        echo "${machine_tag} ${app} ${version} not exists; switch beta version fail !"
...
#~ 记录日志
if [ $? -eq 0 ]; then
    ssh app10-089 "echo "${machine_tag}:`date`:${appType} beta success" >> ${log_path}/release-beta.log"
else
    ssh app10-089 "echo "${machine_tag}:`date`:${appType} beta failure" >> ${log_path}/release-beta.log"
fi
